...
def save_results(save_loc, distractor_scales, sucess_results, steps_needed, i):
    all_sucess_results = np.array(sucess_results)
    all_steps_needed = np.array(steps_needed)
    results = {
        "distractor_scales": distractor_scales[:i+1],
...
def main():
    ...
    distractor_scales = [1.3, 1.4, 1.5]
    save_loc = f"{DIR_PATH}data/test_scales_1p5_cuboid_distractor_{model_name}"
    ...
    for i, distractor_scale in enumerate(distractor_scales):
        print("target color", target_color)
        np.random.seed(random_seed)
        env.stop()
        #dist_size = list(distractor_scale * np.array([0.047873*2, 0.047873*2, 0.06]))
        #distractor = Shape.create(PrimitiveShape.CYLINDER, size=dist_size, position=[0.81, 0.1, 0.6])
        dist_size = list(distractor_scale * np.array([0.12, 0.06, 0.06]))
        distractor = Shape.create(PrimitiveShape.CUBOID, size=dist_size, position=[0.81, 0.1, 0.6])
        objects = [Shape(target_name), distractor]
        objects[0].set_color(format_color(target_color))
        objects[1].set_color(format_color(target_color))
        ...
        save_results(save_loc, distractor_scales, sucess_results, steps_needed, i)
        print("Saved for scale:", distractor_scale)
    env.shutdown()
